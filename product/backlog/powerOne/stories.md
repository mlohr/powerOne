...
> Sprint ordering: **MVP first** (P1 stories in Sprints 1–13), then P2 (Sprints 14–16), then P3 (Sprints 17–18)
...
| `po_StatusBadge` | US-011 | Display | Lifecycle status colored badge |
| `po_ProgressIndicator` | US-011 | Display | Progress % with color coding |
| `po_EmptyState` | US-011 | Layout | Placeholder with icon and message |
| `po_SidePanel` | US-012 | Container | Slide-in panel (header, body, footer) |
| `po_ConfirmDialog` | US-015 | Modal | Confirmation for destructive actions |
| `po_TaskItem` | US-023 | Display | Checkbox task row with owner |
...
- [ ] **Canvas Component Library** (`po_ComponentLibrary`) created in the solution
- [ ] Component naming convention established: `po_` prefix for all components
...
#### Power Platform Notes
- Create the Component Library as a separate Canvas App in the solution — it is imported/referenced by the main app
...
#### Components
- **Builds `po_UserAvatar`**: Reusable component showing user initials (or photo if available) in a circle. Accepts user record as input. Used in sidebar user menu, OKR list (owner column), task items, and KR list.
...
- **Builds `po_StatusBadge`**: Colored badge accepting a status value and rendering with appropriate color (Draft=gray, Accepted=blue, Active=teal, Done=green, Archived=slate, Cancelled=red). Reused across all list views and forms.
- **Builds `po_ProgressIndicator`**: Accepts a progress percentage (0–100) and displays with color coding (<33% red, 33–66% amber, >66% green). Reused in KR displays, program cards, and metric views.
...
#### Components
- **Builds `po_SidePanel`**: Reusable slide-in panel component with configurable header (title + close button), scrollable body area, and footer (action buttons). Accepts title, width, and visibility as inputs; emits onClose and onSave events. Reused by US-013 (KR form), US-014 (Metric form), US-026 (Program form), US-035 (Ritual form).
...
#### Components
- **Builds `po_ConfirmDialog`**: Modal overlay with title, message, confirm/cancel buttons. Accepts title, message, confirmLabel, and variant (warning/danger) as inputs; emits onConfirm and onCancel events. Reused for task deletion (US-023), cascade link removal (US-016), and any destructive action.
...
#### Components
- **Builds `po_FilterChip`**: Small pill showing active filter label with "x" remove button. Accepts label and filterKey; emits onRemove. Used in filter bars across OKR list, Programs dashboard, and Rituals dashboard.
- **Builds `po_SearchableDropdown`**: Wrapper around ComboBox with search input, multi-select support, and consistent styling. Accepts items collection, displayField, and searchField; emits onSelectionChange. Reused in Objective form, KR form, and Program form dropdowns.
...
#### Components
- **Reuses `po_ObjectiveCard`**: Cascade target objectives are rendered using the same component with a temporary highlight effect (teal glow). Auto-expand triggered via the card's expand state input.
- **Reuses `po_KeyResultCard`**: Cascade source Key Results are rendered using the same component with clickable cascade link indicators.
...
#### Components
- **Builds `po_TaskItem`**: Row component with checkbox, description text, owner avatar (reuses `po_UserAvatar`), and hover-reveal action buttons (edit, delete). Accepts task record; emits onToggle, onEdit, onDelete.
...
#### Components
- **Builds `po_CardContainer`**: Consistent card wrapper with border, shadow, padding, and hover effect. Accepts content slot. Used for Programs dashboard cards, Rituals dashboard cards, and Program detail linked objectives.
- **Reuses**: `po_StatusBadge` (program status), `po_ProgressIndicator` (program progress), `po_EmptyState` (no programs), `po_FilterChip` (active filters)
